...
?>
<div class="sec-title">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320"><path fill="#FFD24C" fill-opacity="0.8" d="M0,192L30,165.3C60,139,120,85,180,74.7C240,64,300,96,360,122.7C420,149,480,171,540,176C600,181,660,171,720,192C780,213,840,267,900,250.7C960,235,1020,149,1080,106.7C1140,64,1200,64,1260,85.3C1320,107,1380,149,1410,170.7L1440,192L1440,0L1410,0C1380,0,1320,0,1260,0C1200,0,1140,0,1080,0C1020,0,960,0,900,0C840,0,780,0,720,0C660,0,600,0,540,0C480,0,420,0,360,0C300,0,240,0,180,0C120,0,60,0,30,0L0,0Z"></path></svg>
        <h2>all recorded attachements</h2>
        <a href="pdf_record_form.php?box=record&Class_ID=<?php echo $Class_ID ?>"">Add  new record attachment</a>
    </div>
    ...
    if ($count > 0) {
    ?>
        <table class="table table-warning table-hover text-center" style="width: 80%; margin: 10px auto; vertical-align: middle;">
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                if ($row['Attachment_Status'] == 1) {
                    ...
                    <tr>
                        ...
                        <th>
                        <audio controls>
                            <source src='uploads/" . $row['Attached'] . "' type='audio/ogg'>
                            <source src='uploads/" . $row['Attached'] . "' type='audio/mpeg'>
                            Your browser does not support the audio element.
                        </audio>
                        </th>
                        <th><a class='btn btn-primary' href='?box=Download&Attachment_ID=" . $row['Attachment_ID'] . "&Class_ID=" . $Class_ID . "'>Download</a></th>
                        <th><a class='btn btn-dark' href='?box=Not_activated&Attachment_ID=" . $row['Attachment_ID'] . "&Class_ID=" . $Class_ID . "'>Not activated</a></th>
...
if (isset($_GET['box'])) {
    if ($_GET['box'] == 'Download') {
        ...
        if (file_exists($filepath)) {
            ...
            header('REFRESH:2;URL=View_record.php?Class_ID=' . $Class_ID);
        }
    ...
    <div class="abs-form">
            ...
            <form id="login-form" class="abs-form-fields" action="" method="post" enctype="multipart/form-data">
                ...
                <h5 class="alert alert-info">please fill all fields</h5>
        ...
        if (isset($_POST['submit'])) {
            if (
                ...
                if (!in_array($extension, ['mp3'])) {
                    echo "You file extension must be .mp3";
